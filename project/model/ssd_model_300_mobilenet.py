...
MNET = MobileNetV2(weights='imagenet', include_top=False, input_shape=(300, 300, 3))
...
def ssd_model_300_mobilenet(num_classes=600, reg=0.00003, inference=False):
    conv9_1 = layers.Conv2D(128, (1, 1), activation=activation, padding='same', kernel_regularizer=l2(reg),
                            kernel_initializer=initializer, name='conv9_1')(MNET.output)
    ...
    # defining locs
    loc_1 = layers.Conv2D(4 * 4, (1, 1), name='1st_bbs')(MNET.layers[-98].output)
    loc_2 = layers.Conv2D(6 * 4, (1, 1), name='2nd_bbs')(MNET.layers[-36].output)
    loc_3 = layers.Conv2D(6 * 4, (1, 1), name='3rd_bbs')(MNET.output)
    ...
    conf_1 = layers.Conv2D(4 * num_classes, (1, 1), name='1st_conf')(MNET.layers[-98].output)
    conf_2 = layers.Conv2D(6 * num_classes, (1, 1), name='2nd_conf')(MNET.layers[-36].output)
    conf_3 = layers.Conv2D(6 * num_classes, (1, 1), name='3rd_conf')(MNET.output)
    ...
    model = Model(inputs=MNET.input, output=[final])
    return model
