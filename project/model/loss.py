...
class SSDLoss2:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.dtypes.cast(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12]), tf.float32) # Output shape: (batch_size, n_boxes)
        localization_loss = tf.dtypes.cast(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8]), tf.float32) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = y_true[:,:,0] # Tensor of shape (batch_size, n_boxes)
        positives = tf.dtypes.cast(tf.reduce_max(y_true[:,:,1:-12], axis=-1), tf.float32) # Tensor of shape (batch_size, n_boxes)
        ...
        n_negative_keep = tf.minimum(tf.maximum(self.neg_pos_ratio * tf.dtypes.cast(n_positive, tf.int32), self.n_neg_min), n_neg_losses)
        ...
        def f2():
            ...
            negatives_keep = tf.dtypes.cast(tf.reshape(negatives_keep, [batch_size, n_boxes]), tf.float32) # Tensor of shape (batch_size, n_boxes)
...
class SSDloss():
    ...
    #    conf_loss_pos_na = conf * positives
    #    conf_loss_pos = tf.where(tf.math.is_nan(conf_loss_pos_na),
    #                             tf.zeros_like(conf_loss_pos_na),
    #                             conf_loss_pos_na)
    #    conf_loss_pos_f = tf.reduce_sum(conf_loss_pos, axis=-1)

    #    conf_loss_neg_na = conf * negatives
    #    conf_loss_neg = tf.where(tf.math.is_nan(conf_loss_neg_na),
    #                             tf.zeros_like(conf_loss_neg_na),
    #                             conf_loss_neg_na)
    #    conf_loss_neg_f = tf.reduce_sum(conf_loss_neg, axis=-1)
...
#    def conf_los2(self, y_true, y_pred):
#        return -tf.reduce_sum(y_true[:, :, :-4]
#                              * tf.math.log(y_pred[:, :, :-4]),
#                              axis=-1)
#
    def conf_loss(self, y_true, y_pred):
        return -tf.reduce_sum(y_true[:, :, :-4]
                              * tf.math.log(
                                  tf.math.maximum(y_pred[:, :, :-4],
                                                  1e-8)
                              ),
                              axis=-1)
